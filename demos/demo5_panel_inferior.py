"""Demo 5 — Panel Inferior Compartido: panel fijo abajo que muestra info al hover."""
import flet as ft
...
def main(page: ft.Page):
    page.title = "Demo 5 — Panel Inferior"
    page.bgcolor = "#f0fdf4"
    page.window.maximized = True
    page.padding = 0

    # ── Panel inferior compartido ──
    panel_icon = ft.Container(
        content=ft.Icon(ft.Icons.INFO_OUTLINE, size=32, color="#94a3b8"),
        width=60, height=60, border_radius=30,
        bgcolor=ft.Colors.with_opacity(0.1, "#94a3b8"),
        alignment=ft.Alignment(0, 0),
        animate=ft.Animation(300, ft.AnimationCurve.EASE_OUT),
    )
    panel_nombre = ft.Text("Pase el cursor sobre una brigada", size=20, weight="w700", color="#1e293b")
    panel_sub = ft.Text("La descripción aparecerá aquí", size=13, color="#94a3b8", weight="w500")
    panel_desc = ft.Text("Seleccione una tarjeta para ver información detallada de la brigada.",
                          size=14, color="#475569")
    panel_bar = ft.Container(width=5, height=70, border_radius=3, bgcolor="#94a3b8",
                              animate=ft.Animation(300, ft.AnimationCurve.EASE_OUT))
    ...
    panel = ft.Container(
        content=ft.Row([
            ...
            panel_icon,
            ft.Container(width=20),
            ft.Column([panel_nombre, panel_sub, ft.Container(height=4), panel_desc], spacing=2, expand=True),
        ], vertical_alignment=ft.CrossAxisAlignment.CENTER),
        bgcolor=ft.Colors.with_opacity(0.95, "white"),
        border_radius=20,
        border=ft.Border.all(1.5, ft.Colors.with_opacity(0.15, "#cbd5e1")),
        shadow=[ft.BoxShadow(blur_radius=40, spread_radius=-4,
            color=ft.Colors.with_opacity(0.15, "#1e293b"), offset=ft.Offset(0, -8))],
    ...
    for cfg in BRIGADAS:
        ...
        top_bar = ft.Container(height=5, border_radius=ft.BorderRadius(3, 3, 0, 0),
        ...
        def _hover(card_ref, color, nombre, sub, desc, icono):
            def handler(e):
                hov = e.data == "true"
                card_ref.scale = 1.05 if hov else 1.0
                card_ref.border = ft.Border.all(2.5 if hov else 1.5,
                    ft.Colors.with_opacity(0.6 if hov else 0.15, color if hov else "#cbd5e1"))
                ...
                if hov:
                    panel_icon.content = ft.Icon(icono, size=32, color=color)
                    panel_icon.bgcolor = ft.Colors.with_opacity(0.12, color)
                    panel_nombre.value = nombre
                    panel_nombre.color = color
                    panel_sub.value = sub
                    panel_sub.color = ft.Colors.with_opacity(0.7, color)
                    panel_desc.value = desc
                    panel_bar.bgcolor = color
                    panel.border = ft.Border.all(1.5, ft.Colors.with_opacity(0.2, color))
                page.update()
            return handler

        card.on_hover = _hover(card, cfg["color"], cfg["nombre"], cfg["sub"], cfg["desc"], cfg["icono"])
        cards.append(card)
    ...
    page.add(ft.Column([
        ...
        ft.Text("Demo 5 — Panel Inferior Compartido", size=28, weight="w800", color="#1e293b", text_align=ft.TextAlign.CENTER),
        ft.Text("Hover → el panel inferior muestra la información de esa brigada", size=15, color="#64748b", text_align=ft.TextAlign.CENTER),
        ft.Container(height=20),
        grid,
        ft.Container(height=20),
        ft.Row([panel], alignment=ft.MainAxisAlignment.CENTER),
