"""Demo 1 — Hover Reveal: la descripción se desliza hacia abajo al pasar el mouse."""
import flet as ft
...
def main(page: ft.Page):
    page.title = "Demo 1 — Hover Reveal"
    page.bgcolor = "#f0fdf4"
    ...
    for cfg in BRIGADAS:
        desc_box = ft.Container(
            content=ft.Column([
                ft.Divider(height=1, color=ft.Colors.with_opacity(0.15, cfg["color"])),
                ft.Container(height=8),
                ft.Row([
                    ft.Icon(ft.Icons.INFO_OUTLINE, size=16, color=cfg["color"]),
                    ft.Text("Descripción", size=12, weight="w700", color=cfg["color"]),
                ], spacing=6),
                ft.Container(height=4),
                ft.Text(cfg["desc"], size=13, color="#475569"),
            ], spacing=0),
        ...
        icon_circle = ft.Container(
            ...
            shadow=[ft.BoxShadow(blur_radius=20, spread_radius=0,
                color=ft.Colors.with_opacity(0.4, cfg["color"]), offset=ft.Offset(0, 8))],
        )
        ...
        card = ft.Container(
            content=ft.Column([top_bar, ft.Container(content=card_col, padding=ft.Padding(24, 20, 24, 20))], spacing=0),
        ...
        def _hover(card_ref, desc_ref, color):
            def handler(e):
                hov = e.data == "true"
                card_ref.scale = 1.05 if hov else 1.0
                desc_ref.height = 120 if hov else 0
                desc_ref.opacity = 1.0 if hov else 0
        ...
        card.on_hover = _hover(card, desc_box, cfg["color"])
        cards.append(card)
    ...
    page.add(ft.Container(
        content=ft.Column([
            ...
            ft.Text("Demo 1 — Hover Reveal", size=28, weight="w800", color="#1e293b", text_align=ft.TextAlign.CENTER),
            ft.Text("Pase el mouse sobre las tarjetas → la descripción se desliza", size=15, color="#64748b", text_align=ft.TextAlign.CENTER),
